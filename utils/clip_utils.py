    ...
    def score(self, img_path, prompt):
        if isinstance(img_path, list):
            ...
            for one_img_path in img_path:
                ...
                with torch.no_grad():
                    ...
                    if isinstance(one_img_path, str):
                        image = (
                            self.preprocess_val(Image.open(one_img_path))
                            .unsqueeze(0)
                            .to(device=self.device, non_blocking=True)
                        )
                    elif isinstance(one_img_path, Image.Image):
                        image = (
                            self.preprocess_val(one_img_path)
                            .unsqueeze(0)
                            .to(device=self.device, non_blocking=True)
                        )
                    else:
                    ...
                    if (
                        True
                    ):  # with eval(f'torch.{self.device}.amp.autocast()'): # cuda or cpu
            ...
            with torch.no_grad():
                ...
                if (
                    True
                ):  # with eval(f'torch.{self.device}.amp.autocast()'): # found not needed
                    outputs = self.model(image, text)
                    image_features, text_features = (
                        outputs["image_features"],
                        outputs["text_features"],
