...
class PreferenceLoss(nn.Module):
    ...
    def forward(self, logits_per_image, num_images, labels):
        ...
        paired_logits_list = [
            logit[:, i]
            for i, logit in enumerate(logits_per_image.split(num_images.tolist()))
        ]
        paired_logits = pad_sequence(
            paired_logits_list, batch_first=True, padding_value=-999
        )
...
class RankingLoss(nn.Module):
    ...
    def forward(self, logits_per_image, num_images, labels, margin=1.0):
        ...
        paired_logits = pad_sequence(
            paired_logits_list, batch_first=True, padding_value=-1
        )
        ...
        mask = (
            torch.triu(torch.ones(diff.shape[1], diff.shape[1]), diagonal=1)
            .bool()
            .detach()
        )

        loss = torch.clamp(
            margin - torch.mul(diff[:, ~mask], diff_label[:, ~mask]), min=0
        ).mean()
        return loss
